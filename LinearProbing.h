...
class LinearHashTable
{
  public:
    ...
    explicit LinearHashTable(int numInts, int interval, int size, int primeLessThanM ) : array( size  )
	{ 
		//inititalization of N I M and R
		  ...
		  maxClusterSize = 0;
		  maxProbes = 0;
		  //this nifty trick clears out any previously created results.txt file
		  ofstream inFile("results.txt", ios::out);
		  inFile.close();
		  //calls make empty to set prep array
		  makeEmpty( ); 
	}

	//sets all values of the array to the enumerated value EMPTY
    void makeEmpty( )
    {
        for( int i = 0; i < M; i++ )
            array[ i ].info = EMPTY;
	...
	//function to put items into the array
	void put(const HashedObj & value)
	{
		...
		for(int count = 0; count < M; count ++)
		{
			...
			//if we have a match (WHICH WE SHOULD NOT FOR LINEAR HASHING)
			if (I > 0 && K == hash)
			{
...
//function for displaying the actual tables
	...
	void outputResults(ofstream & out)
	{
		//this is for the first time through to set up table heading
			if (successInsert <=1 )
			{
				out << "\t\tLinear Probing Analysis (Table Size = " << M << " )" << endl;
